...
const transaction_processor_factory_1 = require("../processors/transaction-processor-factory");
const transaction_type_1 = require("../models/transaction-type");
const DEFAULT_USER_BALANCE = 100;
class TransactionManager {
    constructor() {
        ...
        this.transactionProcessorFactory = new transaction_processor_factory_1.TransactionProcessorFactory();
    }
    credit(transaction) {
    ...
    debit(transaction) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!user)
                throw new Error("User not found");
            const userBalance = yield this.userBalanceManager.get(user.id);
            if (userBalance && userBalance.amount < transaction.amount)
                throw new Error("Insufficient balance");
            else if (!userBalance)
                yield this.userBalanceManager.add(transaction.userId, DEFAULT_USER_BALANCE);
            const existingTx = yield this.getByIdempotencyId(transaction.idempotencyId);
            if (existingTx)
                throw new Error("Transaction already exists");
    ...
    getByIdempotencyId(id) {
        return __awaiter(this, void 0, void 0, function* () {
            // could cache
            return yield this.transactionRepository.getByIdempotencyId(id);
        });
    }
    add(transaction) {
        return __awaiter(this, void 0, void 0, function* () {
            return yield this.transactionRepository.add(transaction);
        });
    }
}
